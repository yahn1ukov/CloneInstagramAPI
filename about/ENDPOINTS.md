...
|URL|Method|Role|Input|Output|Note|
|-|-|-|-|-|-|
...
|```/api/admins/posts```|```GET```|```ADMIN```||```IEnumerable<Post>```<br/>```[```<br/>&emsp;```{```<br/>&emsp;&emsp;```Id: Guid```<br/>&emsp;&emsp;```Content: string```<br/>&emsp;```}```<br/>```]```|```Get posts where fields UserId are null```|
...
|```/api/admins/posts/{postId}```|```DELETE```|```ADMIN```|```Id: Guid```|```true```|```Delete post where field UserId is null```|
